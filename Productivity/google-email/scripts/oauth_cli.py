"""
OAuth CLI for Gmail (installed app flow)

Usage:
  python oauth_cli.py authorize
  python oauth_cli.py auth_url
  python oauth_cli.py finish --code <code>
...
# scopes: adjust to least privilege required for your tasks
SCOPES = [
...
def authorize():
    ...
    try:
        # try local server flow (opens browser and listens for callback)
        creds = flow.run_local_server(port=0)
    except Exception as e:
        # fallback to console flow when run_local_server isn't available
...
def finish_with_code(code: str):
    ...
    # InstalledAppFlow.fetch_token 回傳 dict，轉成 Credentials
    creds_obj = Credentials.from_authorized_user_info(token_response, SCOPES)
    with open(TOKEN_PATH, 'w', encoding='utf-8') as f:
